...
win.data <- windowCounts(k20me1_data$Path_to_bam, param=param, width=150, spacing = 50)

#bin into larger windows for background thresholding
bins <- windowCounts(k20me1_data$Path_to_bam, bin=TRUE, width=10000, param=param)
...
genotype[grepl("OregonR_K20me1", genotype)] <- "OregonR_H4K20me1"
genotype[grepl("OregonR_no_primary", genotype)] <- "OregonR_no_primary"
...
contrast <- makeContrasts(OregonR_H4K20me1-OregonR_no_primary, levels=design)
